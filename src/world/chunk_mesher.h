...
#include "render/mesh_payload.h"

struct ChunkMeshPayload
{
    MeshPayload opaqueMesh{};
    MeshPayload transparentMesh{};
};
...
public:
    ...
    std::shared_ptr<ChunkMeshPayload> generate_mesh();

private:
    std::shared_ptr<const ChunkData> _chunk;
    std::optional<std::array<std::shared_ptr<const ChunkData>, 8>>& _chunkNeighbors;

    [[nodiscard]] std::optional<const Block> get_face_neighbor(int x, int y, int z, FaceDirection face) const;
    bool is_face_visible(int x, int y, int z, FaceDirection face);
    bool is_face_visible_water(int x, int y, int z, FaceDirection face);
    void add_face_to_opaque_mesh(int x, int y, int z, FaceDirection face, MeshPayload& mesh);
    void add_face_to_water_mesh(int x, int y, int z, FaceDirection face, MeshPayload& mesh) const;
